...
def judge_pair(prompt, completion_a, completion_b):
    ...
    out = judge_model.generate(
        input_ids, max_new_tokens=3, do_sample=False, pad_token_id=judge_tokenizer.eos_token_id
    )
    response = judge_tokenizer.decode(out[0, input_ids.shape[1]:], skip_special_tokens=True).strip().upper()
